 ...
 * @param bankMin
 */
function compareMinAppAmt(fundMin,bankMin){
	if(parseFloat(fundMin)>parseFloat(bankMin)){
		return fundMin;
	}
	return bankMin;
}
...
/**
 * 判断用户的风险等级
 * fundRisk：基金风险等级：1-低风险；2-低中风险；3-中风险；4-高中风险；5-高风险；
 * userRisk：客户风险等级：1-保守型；2-稳健型；3-积极型
 * @param fundRisk
 * @param userRisk
 * @returns {Boolean}
 */
function riskRule(fundRisk,userRisk){
	if(fundRisk=="1"){
		return true;
	}else if(fundRisk=="2" || fundRisk=="3"){
		if(userRisk=="1"){
			return false;
		}
		return true;
	}else if(fundRisk=="4" || fundRisk=="5"){
		if(userRisk=="1" || userRisk=="2"){
			return false;
		}
		return true;
	}
...
function Arabia_to_Chinese(Num) {
	...
	if (newchar.indexOf("零万") != -1)
	{
		newchar = newchar.replace("零万", "万");
	}
	if (newchar.indexOf("零亿") != -1)
	{
		newchar = newchar.replace("零亿", "亿");
	}
	if (newchar.indexOf("亿万") != -1)
	{
...
//金额转换
function Arabia_to_Chinese_money(Num) {
	for (i = Num.length - 1; i >= 0; i--) {
		Num = Num.replace(",", "")//替换tomoney()中的“,”
	    Num = Num.replace(" ", "")//替换tomoney()中的空格
	}
